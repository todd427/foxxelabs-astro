...
├── agent/
│   ├── rialu-agent.py       # daemon: heartbeat, git status, process list, action handler
...
5. **Anseo ops inclusion:** anseo-alpha is currently Railway. Project entries are not 1:1 with deployments — one project can have both a Railway deployment and a local dev checkout entry.
...
Standalone HTML file: `todd427/foxxelabs-astro/public/ops-dashboard/index.html`
