# Faire — Product Requirements Document
...
The Irish word for *watch*, *vigil*, *guard*. *Faire oíche* = night watch. The tool keeps watch over projects, machines, and deployments — exactly the right framing for a solo developer running distributed research across multiple machines and cloud platforms.

**Deployment target:** `faire.foxxelabs.ie`  
**Repo:** `todd427/faire`
...
Faire is a personal command centre for a solo developer running interconnected projects across multiple physical machines and cloud platforms. It replaces the cognitive overhead of context-switching between Railway dashboards, Fly.io logs, terminal windows, and scattered notes with a single, always-available interface that shows the current state of everything and enables corrective action without leaving the browser.

The primary view is **Projects** — not deployments, not system metrics. Faire is project-first because the developer's mental model is project-first. Infrastructure visibility exists to support projects, not the other way around.
...
The current answer is a constellation of terminal windows, browser tabs, and memory. Faire replaces that.
...
### Machine agents (ops-agent)
A lightweight Python daemon running on each local machine (Rose, Iris, Daisy, optionally Lava). Each agent:
- Heartbeats every 30s to the Faire hub with: process list (filtered to known project names), git repo states for `/home/Projects/`, and resource stats (CPU / RAM / GPU)
- Exposes a local HTTP endpoint (never public) — the hub is the only caller, authenticated via shared HMAC key
...
Browser (any machine, any location)
  ...
  └── Cloudflare Access (Google OAuth — DNS layer, zero app code)
        ...
        └── faire.foxxelabs.ie  [Fly.io]
              ...
              └── ops-agent receiver  (/api/agent/heartbeat, /api/agent/result)
                    │
        ┌───────────┼───────────┐
        │           │           │
...
1. Create a Cloudflare Access application for `faire.foxxelabs.ie`
...
- `fly.toml`, deploy to `faire.foxxelabs.ie`
- Test suite: unit tests for db layer, integration tests for API endpoints

### Phase 2 — Machine agents
- `ops-agent`: Python daemon, heartbeat endpoint, process filtering, git status, resource stats
...
- `ops-agent` upgrade: OAuth 2.1 (same pattern as `mnemos/server/oauth_provider.py`)
...
├── seed_config.py           # one-time: populate platforms, APIs, initial projects
├── main.py                  # FastAPI app, mounts routers, serves static
├── db.py                    # SQLite init, migrations, connection helper
├── poller.py                # APScheduler, all platform API calls
├── routers/
│   ├── projects.py          # /api/projects, /api/milestones, /api/sessions
│   ├── worklog.py           # /api/worklog
│   ├── deployments.py       # /api/deployments
│   ├── machines.py          # /api/machines, /api/agent/*
│   └── budget.py            # /api/budget, /api/apis, /api/api-usage
├── agent/
│   ├── ops-agent.py         # daemon: heartbeat, git status, process list, action handler
│   ├── ops-agent.service    # systemd unit file
│   └── README.md            # install instructions per machine
├── static/
│   └── index.html           # full single-file SPA (all tabs, all themes)
└── tests/
    ├── test_db.py
    ├── test_projects.py
    ├── test_worklog.py
...
2. **Theme persistence:** `localStorage` under key `faire-theme`, dark default. Four themes: dark, light, slate, terminal.
...
5. **Anseo ops inclusion:** anseo-alpha is currently Railway. Should Anseo project link to both the Railway deployment *and* a local development entry on whichever machine has the dev checkout? Yes — project entries are not 1:1 with deployments.

---

## Wireframe

Interactive wireframe deployed at: `foxxelabs.ie/ops-dashboard/`  
Standalone HTML file in repo: `todd427/foxxelabs-astro/public/ops-dashboard/index.html`

---

*Faire PRD v1.0 — FoxxeLabs — 2026-03-19*
